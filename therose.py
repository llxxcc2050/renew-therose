...
# ==================== 配置区域 ====================
# 建议优先在 GitHub Actions 的 Secrets 中配置，本地运行可直接读取环境变量或填入默认值
...
def run_automation():
    ...
    # 使用 SeleniumBase 的 SB 统一管理上下文，自动开启代理支持
    # 注：根据你的日志，代理策略在外面或者通过配置已启用 socks5://127.0.0.1:1080
    with SB(uc=True, headless=True, proxy="socks5://127.0.0.1:1080") as sb:
        try:
            ...
            # 1. 登录主站流程
            log("🌐 打开登录页面...")
            sb.open("https://therose.cloud/login") # 根据实际主站登录页微调
            ...
            log("🛡 处理 Turnstile...")
            # SeleniumBase UC 模式会自动处理或等待 Turnstile，这里加上显式等待
            sb.sleep(3)
            log("✅ Turnstile 验证已处理")
            log("⏳ 等待验证 token 生效...")
            sb.sleep(2)
            ...
            # 2. 续期流程
            log("📄 开始续期流程...")
            # 检查是否存在 Extend 按钮（通常面板包含 extend 关键字）
            ...
            # 【核心修复】：多策略强力按钮匹配与点击方案
            # 兼顾大小写、中英文、类名、以及非 button 标签的热区包裹
            selectors = [
            ...
            # 如果常规点击全部失效，采用终极武器：JavaScript 注入穿透点击
            if not btn_clicked:
